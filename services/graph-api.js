...
module.exports = class GraphApi {
  static async callSendApi(requestBody) {
    ...
    url.search = new URLSearchParams({
      ...
      // access_token: pageAccesToken.data.token
    });
    ...
    if (!response.ok) {
      console.warn(`Could not sent message.`, response.statusText, requestBody);
    }
  }

  static async getUserProfile(senderIgsid) {
    // let pageAcces = await fetch(`${config.tokenUrl}?object=ig_user&id=${receiverIgsid}`)
    // let pageAccesToken = await pageAcces.json();
    // console.log(receiverIgsid);
  ...
  // static async getPageAccessToken(receiverIgsid) {
